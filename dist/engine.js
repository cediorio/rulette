...
class RuleEngine {
  ...
  parseFacts() {
    this.debug("running parseFacts..."); // presently have nothing to do here, but may need to
  ...
  parseRule(name, rule) {
    ...
    rule.lhs = rule.lhs.replace(/ +not +/g, " !"); // add the name as a property for results processing
  ...
  evalRule(rule, facts = null) {
    ...
    if (assignments) {
      for (let i of assignments) {
        ...
        if (lexed) {
          let key = lexed[1];
          let val = lexed[3]; // need to convert any js truth values that may 
          ...
          if (val === 'true') {
            ...
            val = false;
          } else if (val.includes('none', 'None', 'null', 'Null')) {
            val = null;
          ...
          let value = val === fact_value; // now replace the var from the clause with its
          // truth value
