...
exports.RuleEngine = exports.rules = exports.facts = void 0;
var dev = true;
...
var eeval = expr => Function('"use strict";return (' + expr + ')')(); // function eeval(expr){ eval(expr); }
...
class RuleEngine {
  ...
  parseFacts() {
    debug("running parseFacts..."); // presently have nothing to do here, but may need to
  ...
  evalRules() {
    ...
    Object.keys(this.rules).forEach(key => {
      ...
      rule.lhs = rule.lhs.replace(/ +not +/g, " ! "); // use JSON methods to clone a new object, so you
  ...
  evalRule(rule) {
    ...
    if (assignments) {
      for (let i of assignments) {
        ...
        if (lexed) {
          let key = lexed[1];
          let val = lexed[2]; // evaluate the rule's required value against the
          // fact value

          let value = val === this.facts[key].value ? true : false; // now replace the var from the clause with its
          // truth value
...
//# sourceMappingURL=lamekb.js.map